...
export default function PBRWallet({ userDashboardData, refreshDashboardData  }) {
    ...
    const handleFinalizeWallet = () => {
        setIsCreateWalletPopupOpen(false); 
    };
    ...
    if (isLoading) { return (<TransactionPendingOverlay />) }
    ...
    return (
        <>
            {
                userDashboardData?.tonWalletAddress === "TON wallet not linked" ? (
                    !token && isConnected ? (
                        <div>--</div>
                    ) : (
                        <>
                    ...
                    <div className="flex items-center">
                        ...
                        <a href={`https://tonscan.org/address/${tonPbrWalletAddress}`} className="flex items-center">
                            ...
                            {userDashboardData ? (
                                <div className="flex">
                                    <span className="mr-1 bg-pbr-yellow-dark px-2 rounded-lg flex items-center whitespace-nowrap overflow-hidden text-ellipsis max-w-full">
                                        {userDashboardData.tonWalletAddress != "TON wallet not linked" ? `PBR Wallet: ${userDashboardData.tonWalletAddress.slice(0, 6)}...${userDashboardData.tonWalletAddress.slice(-4)}` : ''}
                                    </span>

                                    <span className="text-xl text-gray-900">: {userDashboardData.totalReceivedBearTokens} PBR</span>
                                </div>
